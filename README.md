...
我测试发现移动网络的IP地址，如果UDP数据包长度介于1473-1479、2953-2959、4433-4439。。。等之间，
传输时最后一个IP分片<8字节，这个分片会被丢弃，导致UDP通信受阻，如下所示(从位于电信的某个IP地址测试)。

初步推测该故障是某台路由器，处理ACL时，未正确处理 OFFSET<>0 且 长度<8字节 这种情况引起的。
